...
class StartPage(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        pasw = StringVar()
        pasw_entry = Entry(self, width=13, textvariable=pasw, show="*")
        pasw_entry.grid(column=1, row=2)

        psw_wrong_label = Label(self, text="Contraseña incorrecta")

        button = Button(self, text="Acceder",
                        command=lambda: controller.show_frame(PageOne) if acceso(pasw.get()) is True else psw_wrong_label.grid(column=2, row=2))
...
class PageOne(Frame):
    ...
    def calculate(self):
        try:
            ...
            ## Hacer de otra forma ##
            # self.result.set('Valor al comprar: ' + str(f_compra.round(6)) + '\n' + 'Valor al vender: ' + str(f_venta.round(6)))
